...
export const CommentList: React.FC<Props> = React.memo(({ selected }) => {
  const [
    {
      comments = [],
      doc,
  ...
  const { execCmd, editorState: state } = useEditorStore()
  ...
  const deleteComment = useCallback(
    (id: string, target?: string) => {
      return deleteModel(id)
        ...
        .finally(() => {
          if (target && target.startsWith('MPHighlight:')) {
            execCmd(deleteHighlightMarkers(target))
          }
    ...
    [deleteModel, newComment, setCommentTarget, state]
  )
