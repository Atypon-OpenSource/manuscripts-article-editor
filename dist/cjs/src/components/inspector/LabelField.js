...
const LabelField = ({ value, handleChange, label = 'Label', placeholder }) => {
    const [currentValue, setCurrentValue] = react_1.useState(value);
    const handleValueChange = react_1.useCallback((event) => {
        setCurrentValue(event.target.value);
    }, []);
    const debouncedCurrentValue = use_debounce_1.useDebounce(currentValue, 1000);
    react_1.useEffect(() => {
    ...
    return (react_1.default.createElement(ManuscriptStyleInspector_1.InspectorField, null,
        ...
        react_1.default.createElement(inputs_1.MediumTextField, { value: currentValue, placeholder: placeholder, onChange: handleValueChange })));
};
