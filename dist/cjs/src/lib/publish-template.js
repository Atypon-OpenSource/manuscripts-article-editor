...
exports.buildTemplateModels = void 0;
...
const buildTemplateModels = (manuscript, template, modelMap) => {
    var _a, _b;
    const output = [];
    ...
    const addManuscriptCountRequirements = (manuscript, template) => {
        for (const requirementField of requirements_1.manuscriptCountRequirementFields) {
            ...
            let requirement;
            if (requirementID) {
                requirement = modelMap.get(requirementID);
            }
            ...
            if (requirement && requirement.ignored === false) {
                const objectType = requirement.objectType;
                // TODO: fromPrototype?
    ...
    const addSectionCountRequirements = (section, sectionDescription) => {
    ...
    // sections
    for (const model of modelMap.values()) {
        if (manuscript_1.isSection(model)) {
            const elementIDs = model.elementIDs || [];
            // TODO: section.placeholderTitle?
            const placeholder = elementIDs.length
                ? (_a = modelMap.get(elementIDs[0])) === null || _a === void 0 ? void 0 : _a.placeholderInnerHTML
                : undefined;
            const sectionDescription = {
                _id: manuscript_transform_1.generateID(manuscripts_json_schema_1.ObjectTypes.SectionDescription),
                objectType: manuscripts_json_schema_1.ObjectTypes.SectionDescription,
                sectionCategory: model.category ||
                    (elementIDs.length === 1
            ...
            for (const elementID of elementIDs) {
                ...
                if (model && manuscript_1.isSection(model)) {
                    const title = model.title;
                    if (title) {
                        const elementIDs = model.elementIDs || [];
                        // TODO: section.placeholderTitle?
                        const placeholder = elementIDs.length
                            ? (_b = modelMap.get(elementIDs[0])) === null || _b === void 0 ? void 0 : _b.placeholderInnerHTML
                            : undefined;
            ...
            const requirement = {
                _id: manuscript_transform_1.generateID(manuscripts_json_schema_1.ObjectTypes.MandatorySubsectionsRequirement),
                objectType: manuscripts_json_schema_1.ObjectTypes.MandatorySubsectionsRequirement,
                evaluatedObject: template._id,
                severity: 0,
...
exports.buildTemplateModels = buildTemplateModels;
