...
class RxDBDataBridge {
    constructor(props) {
        ...
        this.dependsOnStateCondition = (condition, dependant, once = true) => {
        ...
        this.subscribe = (manuscriptID, containerID, userID) => {
            ...
            this.dependsOnStateCondition((state) => state.user && state.collaborators, (state) => {
                this.setState({
            ...
            this.dependsOnStateCondition((state) => state.invitations && state.containerInvitations, (state) => {
                this.setState({
            ...
            this.dependsOnStateCondition((state) => state.globalLibraries && state.globalLibraryCollections, (state) => {
                const channels = [
                    ...[
            ...
            this.expect('biblio');
            this.dependsOnStateCondition((state) => state.library &&
                state.library.size &&
                state.manuscript &&
                state.modelMap, (state) => {
                this.cc();
                const bundle = state.manuscript.bundle // TODO: infer bundle from prototype if manuscript.bundle is undefined ?
                    ? state.modelMap.get(state.manuscript.bundle)
                    : null;
                this.biblio = new Bibilo_1.Biblio(bundle, state.library, this.cc(), state.manuscript.lang);
    ...
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                this.dependsOnStateCondition((state) => {
                    if (
                    ...
                    */
                    const modelManager = new ModelManager_1.default(this.state.modelMap, (modelMap) => {
