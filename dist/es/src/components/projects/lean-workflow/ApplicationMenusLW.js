...
const ApplicationMenusLW = ({ editor, contentEditable, }) => {
    ...
    let editorMenu = manuscript_editor_1.getMenus(editor, openDialog, config_1.default.features.footnotes, contentEditable);
    if (!can.formatArticle) {
        editorMenu = editorMenu.filter((menu) => menu.id !== 'format');
    }
    const menu = [...editorMenu, helpMenu];
    if (config_1.default.features.projectMenu) {
